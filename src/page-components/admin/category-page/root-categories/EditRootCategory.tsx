...
export const EditRootCategory = ({ category, updateRootCategory, setIsOpen }: IProps) => {
    const [text, setText] = useState(category.title);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (text.length > 0) {
            updateRootCategory(text, category._id)
            setText('');
            if (setIsOpen) {
                setIsOpen(false)
            }
    ...
    return <div className="max-w-screen-xl px-4 py-16 mx-auto sm:px-6 lg:px-8">
        <div className="max-w-lg mx-auto text-center">
            ...
            <p className="mt-4 text-gray-500">
                Edit Root Category!(This is the base category)
            </p>
        </div>
